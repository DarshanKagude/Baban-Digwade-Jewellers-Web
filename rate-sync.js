...
// Subscribe to real-time rate updates from Realtime Database
...
// Subscribe to hero banner updates from Firebase
const heroBannerRef = ref(db, 'settings/heroBanner');

onValue(heroBannerRef, (snapshot) => {
    if (snapshot.exists()) {
        const bannerData = snapshot.val();
        window.heroBanner = bannerData;
        console.log("Hero banner loaded from Firebase:", bannerData);
    } else {
        window.heroBanner = null;
        console.log("No hero banner in Firebase.");
    }
    // Dispatch event so index.html can inject the carousel slide
    window.dispatchEvent(new CustomEvent('heroBannerReady', { detail: window.heroBanner }));
}, (error) => {
    console.error("Error listening to hero banner:", error);
});
